    ...
    <div className="pb-24">
      <div className="mb-16">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-lg text-zinc-500 font-light max-w-xl">Your personnel clearance, credentials, and system identifiers.</p>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-12">
        ...
        <div className="space-y-2">
          {[
            { label: "Profile Identity", icon: User, active: true, href: "#" },
            { label: "Authentication", icon: KeyRound, active: false, href: "#" },
          ].map(item => (
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white/[0.01] border border-white/[0.03] rounded-[40px] p-10">
            ...
            <div>
              ...
              <div className="w-full bg-black/40 border border-white/[0.05] rounded-xl px-5 py-4 font-mono text-zinc-400 text-sm break-all">{user?.uid || "generating..."}</div>
              <p className="text-xs text-zinc-600 mt-3 font-medium">Binds your identity to the Financial Memory Graph. Do not share.</p>
            </div>
          </div>

          {/* Redirect to Integrations for Gmail */}
          <div className="bg-white/[0.01] border border-indigo-500/10 rounded-[40px] p-8 flex items-center justify-between gap-6">
            <div>
              <p className="font-bold text-white mb-1">Gmail & Integrations</p>
              <p className="text-zinc-500 text-sm">Connect Gmail and manage all integrations from the Ecosystem Hub.</p>
            </div>
            <Link href="/dashboard/integrations" className="shrink-0 px-5 py-3 rounded-xl bg-white text-black text-sm font-bold hover:bg-zinc-100 transition-colors">
              Go to Integrations →
            </Link>
          </div>
