...
export default function GlobalVoiceCopilot() {
  ...
  useEffect(() => {
    // ---- DEBUG LOGS ----
    console.log("[VoiceCopilot Stage]: Listening =", listening);
    if (transcript) console.log("[VoiceCopilot Transcript Updater]:", transcript);
    // --------------------

    if (listening && transcript.length > 0) {
      console.log("[VoiceCopilot Stage]: Active speech detected! Starting 1.5s silence countdown...");
      if (typingTimerRef.current) clearTimeout(typingTimerRef.current);
      typingTimerRef.current = setTimeout(() => {
        console.log("[VoiceCopilot Stage]: 1.5s silence reached. Stopping mic & executing query.");
  ...
  const handleAnalyzeQuery = async (query: string) => {
    console.log("[VoiceCopilot Stage]: Submitting query to LLM:", query);
    if (!query) return;
  ...
  const toggleListen = () => {
    console.log("[VoiceCopilot Stage]: Mic button toggled manually. Was listening?", listening);
    if (listening) {
      SpeechRecognition.stopListening();
      if (transcript.trim().length > 0) {
        handleAnalyzeQuery(transcript);
      } else {
        console.warn("[VoiceCopilot Stage]: Mic stopped manually, but transcript was absolutely empty. Assuming hardware blocked STT or no words spoken.");
      }
    } else {
      setIsOpen(true);
      resetTranscript();
